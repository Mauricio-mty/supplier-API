...
export class RendimientosService {
  ...
  async createFromOrderItemId(purchaseOrderItemId: string): Promise<Rendimiento> {
    ...
    if (existingRendimiento) {
      const rendimientoEditado = this.rendimientosRepository.merge(existingRendimiento, rendimientoDto);
  ...
  private async getPerformanceSource(purchaseOrderItemId: string): Promise<PerformanceSourceRow> {
    const query = `
      SELECT
    ...
    const resultado = (await this.dataSource.query(query, [purchaseOrderItemId])) as PerformanceSourceRow[];

    if (!resultado || resultado.length === 0) {
      throw new NotFoundException(`No se encontró el ítem de orden de compra ${purchaseOrderItemId}`);
    }
  ...
  private buildRendimientoDto(source: PerformanceSourceRow): CreateRendimientoDTo {
    ...
    return {
      ...
      total_performance: bodegaPerformance + calidadPerformance + comprasPerformance,
    };
  }
  ...
  private calculateCalidadBase(source: PerformanceSourceRow): number {
    return [source.cumple_calidad ? 70 : 0, source.cumple_norma ? 30 : 0].reduce(
  ...
  private calculateComprasBase(source: PerformanceSourceRow): number {
    return this.calculateEntregaScore(source.cumple_entrega) + this.calculateServicioScore(source.cumple_servicio);
  }
